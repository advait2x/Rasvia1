...
export type RestaurantOpenStatus = 'open' | 'closed' | 'opening_soon';
 ...
 */
function getCentralTime(): { dayOfWeek: number; minutesSinceMidnight: number; now: Date } {
    const now = new Date();
 ...
 */
export function getRestaurantStatus(hours: RestaurantHour[]): RestaurantStatusResult {
    if (!hours || hours.length === 0) {
        return { status: 'closed', label: 'Hours unavailable', minutesUntilChange: null };
    }
    ...
    for (const shift of todayShifts) {
        ...
        if (minutesSinceMidnight >= openMin && minutesSinceMidnight < closeMin) {
            ...
            return {
                status: 'open',
                label: `Open until ${formatMinutesToAmPm(closeMin)}`,
